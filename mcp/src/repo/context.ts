...
export function getSessionContextHintMessage(
  sessionId: string,
): ModelMessage | undefined {
  const state = loadSessionContextState(sessionId);
  if (!state) return undefined;
  const rendered = renderSessionContextState(state);
  if (!rendered) return undefined;
  return {
    role: "user",
    content:
      "Session memory from prior context pressure. Use this for continuity, but prioritize the current request.\n\n" +
      rendered,
  };
}
...
export async function updateSessionOverflowContext(
  ...
  try {
    const existing =
      loadSessionContextState(sessionId) ?? emptySessionContextState();
    const result = await generateText({
      model,
      prompt: buildUpdatePrompt(existing, newMessages),
      output: Output.object({
        schema: jsonSchema(
          ensureAdditionalPropertiesFalse(SESSION_CONTEXT_SCHEMA),
        ),
      }),
    });
    const nextState = {
      ...normalizeSessionContextState(
        deepParseJsonStrings(result.output ?? {}),
      ),
    ...
    console.error(
      "[context] Failed to update session overflow context:",
      error,
    );
    return undefined;
  }
