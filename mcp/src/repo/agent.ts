...
- \`get_ontology_type\` — Fetch the full schema for a single node type (attributes with type + required/optional status). Use this instead of \`get_ontology\` when you already know the type name and only need field-level detail for that one type.
