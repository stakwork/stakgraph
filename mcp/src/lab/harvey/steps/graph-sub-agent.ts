 ...
 * knowledge-graph research question without holding the graph read tools
 * itself.
 */
const READ_ONLY_GRAPH = [
  "graph/get-ontology",
...
using ONLY the graph tools — you have no file or shell access and must
not invent graph contents. Method: search (graph_graph_search) or enter via
known roots, expand with graph_graph_neighbors, then read the relevant nodes
in full with graph_graph_get_batched. Prefer breadth first (list candidates),
...
export default defineStep({
  ...
  description:
    ...
    "knowledge graph (search → neighbors → batched reads) and returns a findings report with ref_ids. It " +
