...
export class Summarizer {
  ...
  async summarizeConcept(
    ...
    // Load feature
    const feature = await this.storage.getConcept(featureId);
    if (!feature) {
      throw new Error(`Concept ${featureId} not found`);
    }

    console.log(`\n📝 Summarizing feature: ${feature.name}`);

    // Get all PRs and commits for this feature
    const allPRs = await this.storage.getPRsForConcept(featureId);
    const allCommits = await this.storage.getCommitsForConcept(featureId);
    ...
    await this.storage.saveConcept(feature);
  ...
  async summarizeModifiedConcepts(
    ...
    for (let i = 0; i < featureIds.length; i++) {
      ...
      const feature = await this.storage.getConcept(featureId);
      if (!feature) {
        console.log(`${progress} Concept ${featureId} not found, skipping`);
        continue;
      }
      ...
      try {
        const usage = await this.summarizeConcept(feature.id, sessionId);
  ...
  async summarizeAllConcepts(repo?: string): Promise<Usage> {
    const features = await this.storage.getAllConcepts(repo);
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      try {
        const usage = await this.summarizeConcept(feature.id);
        totalUsage = normalizeUsage(addUsage(totalUsage, usage));
        console.log(
          `   📊 Input Usage: ${totalUsage.inputTokens.toLocaleString()} tokens. Output Usage: ${totalUsage.outputTokens.toLocaleString()} tokens`,
        );
