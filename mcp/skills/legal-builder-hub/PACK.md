...
description: >
  Finds, evaluates, and installs community legal skills — with a security review gate
  before anything lands in your environment.
tags: [legal]
...
- **Slash commands are unavailable.** Where a skill says to run
  `/legal-builder-hub:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("legal-builder-hub/some-command")` and follow it.
