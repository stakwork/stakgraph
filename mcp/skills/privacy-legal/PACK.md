...
name: privacy-legal
description: >
  Triages processing activities, generates PIAs, reviews DPAs as controller or processor,
  drafts DSAR responses within statutory timelines, and monitors policy drift against
  practice.
tags: [legal]
...
- **Slash commands are unavailable.** Where a skill says to run
  `/privacy-legal:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("privacy-legal/some-command")` and follow it.
