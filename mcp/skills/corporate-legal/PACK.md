...
description: >
  Runs M&A diligence at scale with cited tabular review, builds disclosure schedules and
  closing checklists, drafts board consents and minutes in house format, and tracks entity
...
- **Slash commands are unavailable.** Where a skill says to run
  `/corporate-legal:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("corporate-legal/some-command")` and follow it.
