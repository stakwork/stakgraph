...
name: law-student
description: >
  Drills Socratically, briefs cases, builds outlines, runs bar prep sessions tuned to your
  jurisdiction, grades IRAC practice, and plans the study schedule — without ever writing
  it for you.
tags: [legal]
...
- **Slash commands are unavailable.** Where a skill says to run
  `/law-student:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("law-student/some-command")` and follow it.
