...
description: >
  ...
  malpractice-aware caution, and hands off cases at semester end — built within ABA Formal
  Op. 512.
tags: [legal]
...
- **Slash commands are unavailable.** Where a skill says to run
  `/legal-clinic:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("legal-clinic/some-command")` and follow it.
