...
description: >
  Watches regulatory feeds, diffs new rules against your policy library, tracks comment
  deadlines and open gaps, and writes the digest your team reads Monday morning.
tags: [legal]
...
- **Slash commands are unavailable.** Where a skill says to run
  `/regulatory-legal:some-command`, there is no such command. Either perform that step's
  work directly, or load the correspondingly-named skill with
  `load_skill("regulatory-legal/some-command")` and follow it.
