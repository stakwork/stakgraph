...
fn api_swift_contains_exact_named_nodes() {
    ...
    assert_eq!(out.exit_code, 0);
    assert_eq!(out.stdout.contains("Class: API (29-144)"), true);
    assert_eq!(
        out.stdout.contains("Function: API.getPeopleList (80-103)"),
        true
    );
    assert_eq!(out.stdout.contains("Request: GET /people (86-90)"), true);
    assert_eq!(out.stdout.contains("Request: POST /person (121-125)"), true);
}
...
fn parse_stats_swift_dir() {
    ...
    assert!(out.stdout.contains("Function             37"), "stdout: {}", out.stdout);
