...
- validate_workflow(yaml): STATIC check of workflow YAML without publishing — unknown step types, bad/duplicate ids, depends on unknown ids or cycles, template refs to unknown roots, config that fails the step schema (template-valued fields are skipped), missing subflow targets. Returns errors (would fail/hang at run time) + warnings. Call it before create_workflow / edit_workflow so a typo doesn't become a published version; fix errors, consider warnings.
...
4. Call validate_workflow on the YAML and fix every error (warnings are judgment calls). Then call create_workflow with the final YAML (or edit_workflow to publish a new version of an existing workflow — get_workflow to read it first).
