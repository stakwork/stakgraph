...
// Window options the SPA's picker exposes. The backend accepts any
// Bifrost duration (observability.go > parseWindow: 1h … 1Y, rolling
// from now); this union is the operator-facing subset, not the
// server's validation set.
export type Window = "1h" | "6h" | "24h" | "7d" | "30d";

// Bucket options for the histogram endpoints. Same story: the server
// takes any duration ≥ 1m and ≤ the window; these are the ones the
// SPA offers.
export type Bucket = "1m" | "5m" | "10m" | "1h" | "6h" | "1d";

// Dimension values the histogram endpoints accept (observability.go
// > parseDimensionParam). Phase 11 removed `realm-id` — every row in
// a swarm's logs.db is implicitly for that swarm's realm, and the
// realm is surfaced on the trust-status card instead of as a
// per-row column.
export type Dimension = "agent-name" | "run-id" | "session-id" | "user-id";
