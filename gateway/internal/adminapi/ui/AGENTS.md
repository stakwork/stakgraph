...
| Types     | `src/api/types.ts` is tygo-generated from the Go structs (`make tygo`; CI fails on drift); `src/api/manual.ts` holds the few non-struct types |
...
└── src/
    ...
    │   ├── types.ts      # GENERATED by `make tygo` from internal/adminapi — do not edit
    │   ├── manual.ts     # hand-maintained: Window/Bucket/Dimension unions, ApiError,
    │   │                 #   trust mirrors, Bifrost pass-through chat/usage shapes
...
- **Cap meters** on RunDetail's live-state card read `max_cost_usd` /
  ...
  budgeted run in the chain, this run first. A null cap renders as
...
1. Define the Go response struct in `gateway/internal/adminapi/`
   (or extend an existing one). Exported, named, `json:` tags — tygo
   only emits what it can see.
2. Run `make tygo` from `gateway/` and commit the regenerated
   `src/api/types.ts`. Never hand-edit it: CI's `make tygo-check`
   (`.github/workflows/gateway-check.yml`) fails on any drift. A type
   with no Go struct behind it (a string union, an error envelope)
   goes in `src/api/manual.ts` instead.
