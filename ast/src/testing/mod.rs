...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_python_services() {
    #[cfg(not(feature = "neo4j"))]
    {
        run_fixture_test::<ArrayGraph>("src/testing/python/services", "python", Language::Python).await.unwrap();
        run_fixture_test::<BTreeMapGraph>("src/testing/python/services", "python", Language::Python).await.unwrap();
    }
    #[cfg(feature = "neo4j")]
    {
        use crate::{lang::graphs::Neo4jGraph, testing::annotations::run_fixture_test};
        let graph = Neo4jGraph::default();
        graph.clear().await.unwrap();
        run_fixture_test::<Neo4jGraph>("src/testing/python/services", "python", Language::Python).await.unwrap();
    }
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_ruby() {
    let use_lsp = Language::Ruby.default_do_lsp();
    #[cfg(not(feature = "neo4j"))]
    {
        run_fixture_test_with_lsp::<ArrayGraph>("src/testing/ruby", "ruby", Language::Ruby, use_lsp).await.unwrap();
        run_fixture_test_with_lsp::<BTreeMapGraph>("src/testing/ruby", "ruby", Language::Ruby, use_lsp).await.unwrap();
    }
    #[cfg(feature = "neo4j")]
    {
        ...
        let graph = Neo4jGraph::default();
        graph.clear().await.unwrap();
        run_fixture_test_with_lsp::<Neo4jGraph>("src/testing/ruby", "ruby", Language::Ruby, use_lsp).await.unwrap();
    }
}
...
async fn test_kotlin() {
    #[cfg(not(feature = "neo4j"))]
    {
        run_fixture_test::<ArrayGraph>("src/testing/kotlin", "kotlin", Language::Kotlin).await.unwrap();
        run_fixture_test::<BTreeMapGraph>("src/testing/kotlin", "kotlin", Language::Kotlin).await.unwrap();
    }
    #[cfg(feature = "neo4j")]
    {
        use crate::{lang::graphs::Neo4jGraph, testing::annotations::run_fixture_test};
        let graph = Neo4jGraph::default();
        graph.clear().await.unwrap();
        run_fixture_test::<Neo4jGraph>("src/testing/kotlin", "kotlin", Language::Kotlin).await.unwrap();
    }
}
