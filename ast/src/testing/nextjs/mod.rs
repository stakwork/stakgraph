...
pub async fn test_nextjs_generic<G: Graph + Sync>() -> Result<()> {
    ...
    if use_lsp {
        ...
        assert_eq!(
            functions.len(),
            169,
            "Expected 169 Function nodes without LSP, found {}",
            functions.len()
        );
    }
    ...
    // ── Noisy-pattern fixture assertions ──────────────────────────────────────
    // These document the CURRENT behavior of the pruner so we can reason about
    // what is kept vs filtered before making any pruner changes.
    ...
    // Category 1: inline object in member-expression assignment — NestedIn → Var.
    // global.fetch is now captured as a Var, so json/text get NestedIn → global.fetch.
    assert!(
        noisy_pattern_functions.iter().any(|f| f.name == "json"),
        "Cat1: 'json' survives (NestedIn → Var, Var-protected)"
    );
    assert!(
        noisy_pattern_functions.iter().any(|f| f.name == "text"),
        "Cat1: 'text' survives (NestedIn → Var, Var-protected)"
    );
    #[cfg(not(feature = "neo4j"))]
    assert!(
        edge_vec.iter().any(|e| {
    ...
    assert!(
        edge_vec.iter().any(|e| {
            e.edge == EdgeType::NestedIn
                && e.source.node_data.name == "text"
                && e.source.node_data.file.ends_with(noisy_pattern_file)
        }),
        "Cat1: 'text' has NestedIn edge to global.fetch Var"
    );

    // Category 2: factory callback passed into jest.mock — no NestedIn edge exists.
    assert!(
    ...
    // Category 3: returned object from a tracked function.
    // onSuccess survives (has outgoing Calls → formatNumber).
    // onError is pruned (NestedIn → createApiHandlers Function, no Calls).
    assert!(
    ...
    // Category 4: deeply nested config inside a Var — both survive (NestedIn → Var protected).
    assert!(
    ...
    // Category 5: class method body — getHandlers survives, parse/serialize pruned.
    // parse/serialize have NestedIn → getHandlers (Function), no Calls of their own.
    assert!(
    ...
    edges += contains;
    if use_lsp {
        assert_eq!(contains, 566, "Expected 566 Contains edges with LSP");
    } else {
        assert_eq!(contains, 548, "Expected 548 Contains edges");
    }
    ...
    if let Some(_currency_test) = tests.iter().find(|t| {
        ...
        let all_convert_sats_calls: Vec<_> = edges_vec
            .iter()
            .filter(|e| {
