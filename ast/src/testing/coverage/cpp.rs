...
#[cfg(feature = "neo4j")]
async fn setup_cpp_graph() -> Result<crate::lang::graphs::graph_ops::GraphOps> {
...
async fn test_btreemap_test_to_function_edges() -> Result<()> {
    ...
    assert_eq!(integration_test_to_function_edges.len(), 4);
...
async fn test_coverage_default_params() -> Result<()> {
    let mut graph_ops = setup_cpp_graph().await?;

    let coverage = graph_ops.get_coverage(None, None, None).await?;

    assert_eq!(coverage.language, Some("cpp".to_string()));
...
async fn test_coverage_with_repo_filter() -> Result<()> {
    ...
    let coverage = graph_ops
        .get_coverage(Some("src/testing/cpp"), None, None)
        .await?;
...
async fn test_coverage_with_ignore_dirs() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        e2e_regexes: vec![],
        target_regex: None,
        ignore_dirs: vec!["cuda".to_string()],
    };
    let filtered = graph_ops.get_coverage(None, Some(filters), None).await?;

    assert_eq!(filtered.language, Some("cpp".to_string()));
...
async fn test_coverage_with_regex_filter() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*vector.*".to_string()),
        ignore_dirs: vec![],
    };
...
async fn test_coverage_with_is_muted() -> Result<()> {
    ...
    assert_eq!(muted_coverage.language, Some("cpp".to_string()));
    assert_eq!(unmuted_coverage.language, Some("cpp".to_string()));
...
async fn test_coverage_combined_filters() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*matrix.*".to_string()),
        ignore_dirs: vec!["web_api".to_string()],
    };

    let coverage = graph_ops
        .get_coverage(Some("src/testing/cpp"), Some(filters), Some(false))
        .await?;
