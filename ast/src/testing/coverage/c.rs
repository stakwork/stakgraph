...
#[cfg(feature = "neo4j")]
async fn setup_c_graph() -> Result<crate::lang::graphs::graph_ops::GraphOps> {
...
async fn test_btreemap_test_to_function_edges() -> Result<()> {
    ...
    assert_eq!(unit_test_to_function_edges.len(), 27);
    ...
    assert_eq!(integration_test_to_function_edges.len(), 18);
...
async fn test_coverage_default_params() -> Result<()> {
    let mut graph_ops = setup_c_graph().await?;

    let coverage = graph_ops.get_coverage(None, None, None).await?;

    assert_eq!(coverage.language, Some("c".to_string()));

    if let Some(unit) = &coverage.unit_tests {
        assert_eq!(unit.total_tests, 21);
    }

    if let Some(integration) = &coverage.integration_tests {
        assert_eq!(integration.total_tests, 7);
    }
...
async fn test_coverage_with_repo_filter() -> Result<()> {
    ...
    let coverage = graph_ops
        .get_coverage(Some("src/testing/c"), None, None)
        .await?;
...
async fn test_coverage_with_ignore_dirs() -> Result<()> {
    ...
    assert_eq!(filtered.language, Some("c".to_string()));
...
async fn test_coverage_with_regex_filter() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*memory.*".to_string()),
        ignore_dirs: vec![],
    };
...
async fn test_coverage_with_is_muted() -> Result<()> {
    ...
    assert_eq!(muted_coverage.language, Some("c".to_string()));
    assert_eq!(unmuted_coverage.language, Some("c".to_string()));
...
async fn test_coverage_combined_filters() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*thread.*".to_string()),
        ignore_dirs: vec!["embedded-iot".to_string()],
    };

    let coverage = graph_ops
        .get_coverage(Some("src/testing/c"), Some(filters), Some(false))
        .await?;
