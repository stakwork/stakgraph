...
#[cfg(feature = "neo4j")]
async fn setup_swift_graph() -> Result<crate::lang::graphs::graph_ops::GraphOps> {
...
async fn test_btreemap_test_to_function_edges() -> Result<()> {
    ...
    assert_eq!(calls_edges, 26);

    let contains_edges = graph.count_edges_of_type(EdgeType::Contains);
    assert_eq!(contains_edges, 173);
...
async fn test_coverage_default_params() -> Result<()> {
    let mut graph_ops = setup_swift_graph().await?;

    let coverage = graph_ops.get_coverage(None, None, None).await?;

    assert_eq!(coverage.language, Some("swift".to_string()));
...
async fn test_coverage_with_repo_filter() -> Result<()> {
    ...
    let coverage = graph_ops
        .get_coverage(Some("src/testing/swift"), None, None)
        .await?;
...
async fn test_coverage_with_ignore_dirs() -> Result<()> {
    ...
    assert_eq!(filtered.language, Some("swift".to_string()));
...
async fn test_coverage_with_regex_filter() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*Profile.*".to_string()),
        ignore_dirs: vec![],
    };
...
async fn test_coverage_with_is_muted() -> Result<()> {
    ...
    assert_eq!(muted_coverage.language, Some("swift".to_string()));
    assert_eq!(unmuted_coverage.language, Some("swift".to_string()));
...
async fn test_coverage_combined_filters() -> Result<()> {
    ...
    let filters = TestFilters {
        ...
        integration_regexes: vec![],
        e2e_regexes: vec![],
        target_regex: Some(".*Chat.*".to_string()),
        ignore_dirs: vec!["LegacyApp".to_string()],
    };

    let coverage = graph_ops
        .get_coverage(Some("src/testing/swift"), Some(filters), Some(false))
        .await?;
