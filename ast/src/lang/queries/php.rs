...
impl Stack for Php {
    ...
    fn q(&self, q: &str, nt: &NodeType) -> Query {
        if matches!(nt, NodeType::Library) {
            match Query::new(&tree_sitter_json::LANGUAGE.into(), q) {
                Ok(query) => query,
                Err(err) => panic!("Failed to compile Php library query '{}': {}", q, err),
            }
        } else {
            match Query::new(&self.0, q) {
                Ok(query) => query,
                Err(err) => panic!("Failed to compile Php query '{}': {}", q, err),
            }
        }
    }
