use super::{cs_resolver, Registry};
...
impl CSharpRegistry {
    pub fn new(graph: &impl Graph, filez: &[(String, String)]) -> Self {
        ...
        for (node_type, node_data) in graph.iter_all_nodes() {
            if !node_data.file.ends_with(".cs") {
                continue;
            }
            match node_type {
                NodeType::Function => {
        ...
        for (file, source) in filez {
            ...
            let fields = cs_resolver::extract_class_fields(source);
            ...
            let returns = cs_resolver::extract_method_return_types(source);
        ...
        let all_resolved: Vec<((String, usize, usize), NodeKeys)> = filez
            .iter()
            .filter(|(f, _)| f.ends_with(".cs"))
            .flat_map(|(file, source)| {
                cs_resolver::resolve_file_calls(
                    source,
                    file,
                    &reg.class_fields,
                    &reg.method_returns,
...
impl Registry for CSharpRegistry {
