...
/// Extract method return types from Go source.
/// Returns { (receiver_type, method_name) → base_return_type }.
pub fn extract_method_return_types(source: &str) -> HashMap<(String, String), String> {
    ...
    for i in 0..root.named_child_count() {
        ...
        extract_method_return_from_node(node, src, &mut out);
    }
    out
}

fn extract_method_return_from_node(
    node: Node,
    source: &[u8],
    out: &mut HashMap<(String, String), String>,
) {
    // Receiver: (parameter_list (parameter_declaration type: T | *T))
    let Some(recv_list) = node.child_by_field_name("receiver") else {
        return;
    };
    let Some(pd) = recv_list.named_child(0) else {
        return;
    };
    if pd.kind() != "parameter_declaration" {
        return;
    }
    let Some(type_node) = pd.child_by_field_name("type") else {
        return;
    };
    let Some(recv_type) = strip_go_type(type_node, source) else {
        return;
    };
    ...
    let base_type: Option<String> = match result_node.kind() {
        ...
        "parameter_list" => (0..result_node.named_child_count())
            .filter_map(|j| result_node.named_child(j))
            .find_map(|child| {
                let tn = if child.kind() == "parameter_declaration" {
                    child.child_by_field_name("type")?
                } else {
                    child
                };
                let t = strip_go_type(tn, source)?;
                if t == "error" { None } else { Some(t) }
            }),
        _ => None,
    };

    if let Some(t) = base_type {
        out.entry((recv_type, method_name.to_string())).or_insert(t);
    }
}
...
fn eval_expr_type(
    ...
    match node.kind() {
        ...
        "unary_expression" => {
            ...
            node.named_child(0).and_then(|inner| {
                eval_expr_type(scope, struct_fields, fn_returns, method_returns, inner, source)
            })
        }
        "composite_literal" => {
        ...
        "call_expression" => {
            ...
            match func.kind() {
                ...
                "selector_expression" => {
                    ...
                    let method_name =
                        func.child_by_field_name("field")?.utf8_text(source).ok()?;
                    method_returns
...
fn try_bind_short_var(
    ...
    let types: Vec<Option<String>> = (0..right.named_child_count())
        ...
        .map(|n| eval_expr_type(scope, struct_fields, fn_returns, method_returns, n, source))
        .collect();
    ...
    match node.kind() {
        "function_declaration" => {
            ...
            if let Some(body) = node.child_by_field_name("body") {
                walk_node(
                    body, source, scope, struct_fields, fn_returns, method_returns, pkg_fns,
                    graph, out, file,
                );
            }
            scope_pop(scope);
        ...
        "short_var_declaration" => {
            ...
            try_bind_short_var(node, source, scope, struct_fields, fn_returns, method_returns);
            if let Some(right) = node.child_by_field_name("right") {
                walk_node(
                    right, source, scope, struct_fields, fn_returns, method_returns, pkg_fns,
                    graph, out, file,
                );
            }
        }
        "call_expression" => {
            if let Some(func_node) = node.child_by_field_name("function") {
                if let Some(target) = resolve_callee(
                    scope,
                    struct_fields,
                    fn_returns,
                    method_returns,
                ...
                walk_node(
                    func_node, source, scope, struct_fields, fn_returns, method_returns, pkg_fns,
                    graph, out, file,
                );
                // Recurse into arguments for nested calls within args
                if let Some(args) = node.child_by_field_name("arguments") {
                    walk_node(
                        args, source, scope, struct_fields, fn_returns, method_returns, pkg_fns,
                        graph, out, file,
                    );
                }
            }
        ...
        _ => {
            for i in 0..node.named_child_count() {
                if let Some(child) = node.named_child(i) {
                    walk_node(
                        child, source, scope, struct_fields, fn_returns, method_returns, pkg_fns,
                        graph, out, file,
                    );
                }
            }
