...
impl Graph for ArrayGraph {
    ...
    async fn add_node(&mut self, node_type: NodeType, node_data: NodeData) {
        let node = Node::new(node_type, node_data);
        let key = create_node_key(&node);
        if !self.node_keys.contains(&key) {
            self.node_keys.insert(key);
            self.nodes.push(node);
    ...
    async fn add_node_with_parent(
            ...
            .map(|n| n.node_data.clone())
        {
            ...
            self.add_node(node_type, node_data).await;
    ...
    async fn process_endpoint_groups(&mut self, eg: Vec<NodeData>, lang: &Lang) -> Result<()> {
        ...
        for group in eg {
            ...
            if let Some(g) = group.meta.get("group") {
                ...
                if let Some(gf) = self
                    .find_nodes_by_name(NodeType::Function, &g)
                    .await
                    .first()
                {
            ...
            if n.node_type == NodeType::Class {
                if let Some(parent) = n.node_data.meta.get("parent") {
                    if let Some(parent_node) = self
                        .find_nodes_by_name(NodeType::Class, parent)
                        .await
                        .first()
                    {
        ...
        for n in self.nodes.iter() {
            if n.node_type == NodeType::Class {
                if let Some(includes) = n.node_data.meta.get("includes") {
                    ...
                    for m in modules {
                        if let Some(m_node) =
                            self.find_nodes_by_name(NodeType::Class, m).await.first()
                        {
    ...
    async fn add_instances(&mut self, instances: Vec<NodeData>) {
        for inst in instances {
            if let Some(of) = &inst.data_type {
                if let Some(cl) = self.find_nodes_by_name(NodeType::Class, &of).await.first() {
    ...
    async fn add_functions(&mut self, functions: Vec<Function>) {
        for f in functions {
            ...
            self.add_node(NodeType::Function, node.clone()).await;
            if let Some(p) = method_of {
                self.add_edge(p.into()).await;
            }
            if let Some(to) = trait_operand {
                self.add_edge(to.into()).await;
            }
            for rt in return_types {
                self.add_edge(rt).await;
            }
            for r in reqs {
                let edge = Edge::calls(NodeType::Function, &node, NodeType::Request, &r);
                self.add_edge(edge).await;
                self.add_node(NodeType::Request, r).await;
            }
            for dm in dms {
                self.add_edge(dm).await;
            }
        }
    }
    async fn add_page(&mut self, page: (NodeData, Option<Edge>)) {
        let (p, e) = page;
        self.add_node(NodeType::Page, p).await;
        if let Some(edge) = e {
            self.add_edge(edge).await;
        }
    }
    ...
    async fn add_endpoints(&mut self, endpoints: Vec<(NodeData, Option<Edge>)>) {
        for (e, h) in endpoints {
            if let Some(_handler) = e.meta.get("handler") {
                ...
                if self.find_endpoint(&e.name, &e.file, verb).await.is_some() {
                    continue;
                }
                self.add_node(NodeType::Endpoint, e).await;
                if let Some(edge) = h {
                    self.add_edge(edge).await;
                }
            } else {
    ...
    async fn add_calls(
        ...
        for (fc, ext_func, class_call) in funcs {
            ...
            if let Some(ext_nd) = ext_func {
                ...
                if !unique_edges.contains(&edge_key) {
                    ...
                    self.add_edge(Edge::uses(fc.source, &ext_nd)).await;
                    ...
                    if self
                        ...
                        .is_none()
                    {
                        self.add_node(NodeType::Function, ext_nd).await;
                    }
                }
            } else {
                if let Some(target_function) = self
                    .find_node_by_name_in_file(NodeType::Function, &fc.target.name, &fc.source.file)
                    .await
                {
                    let edge_key = (
                        fc.source.name.clone(),
        ...
        for (tc, ext_func, _) in tests {
            if let Some(ext_nd) = ext_func {
                ...
                if !unique_edges.contains(&edge_key) {
                    unique_edges.insert(edge_key);
                    self.add_edge(Edge::uses(tc.source, &ext_nd)).await;
                ...
                if !unique_edges.contains(&edge_key) {
                    unique_edges.insert(edge_key);
                    self.add_edge(Edge::new_test_call(tc)).await;
                }
            }
        }
    ...
    async fn find_handlers_for_endpoint(&self, endpoint: &NodeData) -> Vec<NodeData> {
        ...
        if let Some(endpoint) = endp_node {
            ...
            for edge in &self.edges {
                ...
                {
                    let handler_nodes = self
                        .find_nodes_by_name(
                            edge.target.node_type.clone(),
                            &edge.target.node_data.name,
                        )
                        .await;
                    if let Some(handler) = handler_nodes.first() {
...
impl ArrayGraph {
    ...
    // Synchronous helper for lang API compatibility
    fn find_nodes_by_file_ends_with_sync(&self, node_type: NodeType, file: &str) -> Vec<NodeData> {
        self.nodes
            .iter()
            .filter(|node| node.node_type == node_type && node.node_data.file.ends_with(file))
            .map(|node| node.node_data.clone())
