...
pub struct Neo4jGraph {
    connection: Arc<Mutex<Option<Arc<Neo4jConnection>>>>,
    config: Neo4jConfig,
...
impl Neo4jGraph {
    ...
    pub async fn connect(&self) -> Result<()> {
        ...
        if *conn_status && conn_guard.is_some() {
    ...
    pub async fn ensure_connected(&self) -> Result<Arc<Neo4jConnection>> {
        ...
        conn_guard.as_ref().cloned().ok_or_else(|| {
            anyhow::anyhow!("Connection is not established after a successful connect call")
        })
    }
    ...
    pub async fn get_repository_hash(&mut self, repo_url: &str) -> Result<String> {
        ...
        let result = connection.execute(query_obj).await;
...
impl Neo4jGraph {
    ...
    pub async fn find_nodes_by_name_async(&self, node_type: NodeType, name: &str) -> Vec<NodeData> {
        if let Ok(connection) = self.ensure_connected().await {
        ...
        if let Ok(connection) = self.ensure_connected().await {
            ...
            if let Ok(mut result) = connection.execute(query_obj).await {
                if let Ok(Some(row)) = result.next().await {
                    return row.get::<usize>("count").unwrap_or(0);
                }
            }
        }
        0
    ...
    pub async fn check_direct_data_model_usage_async(
        ...
        if let Ok(connection) = self.ensure_connected().await {
            let (query_str, params) =
                check_direct_data_model_usage_query(function_name, data_model);
