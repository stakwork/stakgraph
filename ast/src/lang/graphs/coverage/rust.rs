...
pub(super) async fn get_coverage(
    ...
    let non_empty_in_scope = |n: &NodeData| in_scope(n) && !n.body.trim().is_empty();

    let unit_functions: Vec<NodeData> = functions
        .clone()
        .into_iter()
        .filter(|n| non_empty_in_scope(n))
        .collect();

    let integration_functions: Vec<NodeData> = functions
        .into_iter()
        .filter(|n| non_empty_in_scope(n))
        .collect();
